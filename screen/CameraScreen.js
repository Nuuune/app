...
export default class CameraScreen extends React.Component {
    ...
    handleBase64 = async (path) => {
        return await ImageResizer.createResizedImage(path, Util.px2dp(800), Util.px2dp(600), 'JPEG', 80, 0, "base64");
    }
    ...
    takePicture = () => {
        ...
        if (this.camera) {
            this.camera.takePictureAsync({ quality: 0 }).then(async (result) => {

                const onSaved = navigation.getParam('onSaved', null);
                if (onSaved) {
                    onSaved.call(this, {width: result.width, height: result.height});
                }
            }).catch(err => {
                console.log(err);
            })
        ...
        this.props.navigation.navigate('MAppBox');
    };
